    ...
    <>
      <div className="min-h-[100vh] w-[85vw] m-auto mt-[30px] ">
        ...
        {loading ? (
          ...
          orders?.filter((order)=>order.status == 'pending')?.map((crt, i) => {
            return (
              <div
                key={i}
              ...
              >
                <div className="flex flex-col  w-[400px]">
                  {crt.items.map((item, i) => (
                    <div className="flex   items-center gap-5 p-3 " key={i}>
                      <img
                ...
                <div className="ml-[150px] flex flex-col gap-4 items-end">
                  <p className="font-bold text-sm">
                    Sub-Total :{crt?.subtotal}
                  </p>
                  {user.isAdmin ? (
                    <button
                      onClick={() => updateStatus(crt._id)}
                      className="w-[100px] h-[40px] text-green-500 font-bold border-2 rounded-xl"
                    >
                      {!crt.status == "approved" ? "Approve" : crt.status}
                    </button>
                  ) : (
                    <h1 className="w-[100px] h-[40px] text-yellow-500 font-bold border-2  rounded-xl flex text-center justify-center items-center">
                      {crt?.status}
                    </h1>
                  )}
                </div>
